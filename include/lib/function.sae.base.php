...
function __autoload($class) {
	...
	if($class=='cache') return require_once(EMLOG_ROOT . '/include/lib/cachesae.php');
...
function upload($fileName, $errorNum, $tmpFile, $fileSize, $type, $isIcon = false, $is_thumbnail = true) {
	...
	$file_info['width'] = 0;
	$file_info['height'] = 0;
    $fname = $fileName;//substr(md5($fileName), 0, 4) . time() . '.' . $extension;
	...
	if ($is_thumbnail) {
		if ($isIcon && resizeImage($tmpFile, $thum, Option::ICON_MAX_W, Option::ICON_MAX_H)) {
			$thumarr = explode('<||>', $thum);
			$thum = $thumarr[0];
			$file_info['thum_file'] = $thum;
			$file_info['thum_size'] = $thumarr[1];
			...
			//头像上传
			$iconthum = 'thum52-'.$fname;
			resizeImage($tmpFile,$iconthum, 52, 52);
	...
	if (@is_uploaded_file($tmpFile)) {
		$EmStorage = EmStorage::getInstance();
		$Storageurl = $EmStorage->upload($fname, $tmpFile);
		if(!$Storageurl) {
			@unlink($tmpFile);
			return '105';
		}
		@unlink($tmpFile);
		$file_info['file_path'] = $Storageurl;
	}
...
function imageCropAndResize($src_image, &$dst_path, $dst_x, $dst_y, $src_x, $src_y, $dst_w, $dst_h, $src_w, $src_h) {
	$src_img = file_get_contents($src_image);
	if(!$src_img) return false;
	
	//用 SaeImage 生成缩略图
	$SaeImage = new SaeImage();
	$SaeImage->setData($src_img);
	$SaeImage->resize($dst_w,$dst_h);
	$new_data = $SaeImage->exec();
	$SaeImage->clean();
	$EmStorage = EmStorage::getInstance();
	$Storageurl = $EmStorage->write($dst_path, $new_data);
	$thumsize = strlen($new_data);
	unset($src_img,$new_data);
	if($Storageurl) {
		$dst_path = $Storageurl.'<||>'.$thumsize;
		return true;
	}
	else return false;
}
