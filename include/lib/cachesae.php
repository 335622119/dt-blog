...
class Cache {
	...
	function cacheWrite ($cacheData, $cacheName) {
		...
		$cachekey = self::$prefix . $cacheName;
		self::$kv->set($cachekey,$cacheData) OR emMsg('写入缓存失败，请检查kvdb设置状态');
	...
	function readCache($cacheName) {
		if ($this->{$cacheName.'_cache'} != null) {
			...
			$cachekey = self::$prefix . $cacheName;
			$cacheData = self::$kv->get($cachekey);
			// 如果缓存文件不存在则自动生成缓存文件(kvdb的value值)
			if (!$cacheData || empty($cacheData)) {
				if (method_exists($this, 'mc_' . $cacheName)) {
					call_user_func(array($this, 'mc_' . $cacheName));
				}
			}
			$this->{$cacheName.'_cache'} = unserialize($cacheData);
			return $this->{$cacheName.'_cache'};
		}
	}
}
