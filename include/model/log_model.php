...
class Log_Model {
	...
	function deleteLog($blogId) {
		...
		while ($attach = $this->db->fetch_array($query)) {
			if(strpos($attach['filepath'],'stor.sinaapp.com')!==false) {
				$fpath = str_replace('thum-', '', $attach['filepath']);
				$EmStorage = EmStorage::getInstance();
				if ($fpath != $attach['filepath']) $EmStorage->delete($fpath);
				$EmStorage->delete($attach['filepath']) or emMsg("删除SAEStorage头像图片附件失败!");
			}elseif (file_exists($attach['filepath'])) {
				$fpath = str_replace('thum-', '', $attach['filepath']);
