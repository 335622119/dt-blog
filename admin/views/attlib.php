...
foreach($attach as $key=>$value):
	...
	//增加 sae判断
	$saeflag = strpos($value['filepath'],'stor.sinaapp.com');
	if($saeflag) $atturl = $value['filepath'];
	else $atturl = BLOG_URL.substr($value['filepath'], 3);
	if ($extension == 'zip' || $extension == 'rar'){
		...
		if($saeflag) $ed_imgpath = $value['filepath'];
		else $ed_imgpath = BLOG_URL.substr($imgpath,3);
		...
		if (isset($value['thum_filepath'])) {
			if($saeflag) $thum_url = $value['thum_filepath'];
			else $thum_url = BLOG_URL.substr($value['thum_filepath'], 3);
