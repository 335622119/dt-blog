...
<div id="tw">
	...
	//增加 sae判断
	$saeflag = strpos($val['img'],'stor.sinaapp.com');
	if($saeflag) {
		$img = '<a title="查看图片" href="'.str_replace('thum-', '', $val['img']).'" target="_blank"><img style="border: 1px solid #EFEFEF;" src="'.$val['img'].'"/></a>';
	}else {
		$img = '<a title="查看图片" href="'.BLOG_URL.str_replace('thum-', '', $val['img']).'" target="_blank"><img style="border: 1px solid #EFEFEF;" src="'.BLOG_URL.$val['img'].'"/></a>';
	}
    $img = empty($val['img']) ? "" : $img;
    ?>
    <li class="li">
