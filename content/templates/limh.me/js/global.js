...
$(function() {
	!window.ActiveXObject && document.createElement("canvas").getContext ? $.getScript("../content/templates/limh.me/js/cav.js", function() {
		function a() {
			l.remove(r);
			f.clear();
			p = new CAV.Plane(d.width * f.width, d.height * f.height, d.segments, d.slices);
			v = new CAV.Material(d.ambient, d.diffuse);
			r = new CAV.Mesh(p, v);
			l.add(r);
			var a, b;
			for (a = p.vertices.length - 1; 0 <= a; a--) b = p.vertices[a], b.anchor = CAV.Vector3.clone(b.position), b.step = CAV.Vector3.create(Math.randomInRange(.2, 1), Math.randomInRange(.2, 1), Math.randomInRange(.2, 1)), b.time = Math.randomInRange(0, Math.PIM2)
		}
		function b(b, d) {
			f.setSize(b, d);
			CAV.Vector3.set(q, f.halfWidth, f.halfHeight);
			a()
		}
		function c() {
			h = Date.now() - m;
			var a, b, g, u, n, r = d.depth / 2;
			CAV.Vector3.copy(e.bounds, q);
			CAV.Vector3.multiplyScalar(e.bounds, e.xyScalar);
			CAV.Vector3.setZ(k, e.zOffset);
			for (a = l.lights.length - 1; 0 <= a; a--) b = l.lights[a], CAV.Vector3.setZ(b.position, e.zOffset), g = Math.clamp(CAV.Vector3.distanceSquared(b.position, k), e.minDistance, e.maxDistance), g = e.gravity * b.mass / g, CAV.Vector3.subtractVectors(b.force, k, b.position), CAV.Vector3.normalise(b.force), CAV.Vector3.multiplyScalar(b.force, g), CAV.Vector3.set(b.acceleration), CAV.Vector3.add(b.acceleration, b.force), CAV.Vector3.add(b.velocity, b.acceleration), CAV.Vector3.multiplyScalar(b.velocity, e.dampening), CAV.Vector3.limit(b.velocity, e.minLimit, e.maxLimit), CAV.Vector3.add(b.position, b.velocity);
			for (u = p.vertices.length - 1; 0 <= u; u--) n = p.vertices[u], a = Math.sin(n.time + n.step[0] * h * d.speed), b = Math.cos(n.time + n.step[1] * h * d.speed), g = Math.sin(n.time + n.step[2] * h * d.speed), CAV.Vector3.set(n.position, d.xRange * p.segmentWidth * a, d.yRange * p.sliceHeight * b, d.zRange * r * g - r), CAV.Vector3.add(n.position, n.anchor);
			p.dirty = !0;
			f.render(l);
		...
		var d = {
			...
			e = {
				...
				step: CAV.Vector3.create(Math.randomInRange(.2, 1), Math.randomInRange(.2, 1), Math.randomInRange(.2, 1))
			},
			h, m = Date.now(),
			q = CAV.Vector3.create(),
			k = CAV.Vector3.create(),
			g = document.getElementById("container1"),
			w = document.getElementById("anitOut"),
			f, l, r, p, v, x;
		x = new CAV.CanvasRenderer;
		f && w.removeChild(f.element);
		f = x;
		f.setSize(g.offsetWidth, g.offsetHeight);
		w.appendChild(f.element);
		l = new CAV.Scene;
		a();
		(function() {
			var b, a;
			for (b = l.lights.length - 1; 0 <= b; b--) a = l.lights[b], l.remove(a);
			f.clear();
			for (b = 0; b < e.count; b++) a = new CAV.Light(e.ambient, e.diffuse), a.ambientHex = a.ambient.format(), a.diffuseHex = a.diffuse.format(), l.add(a), a.mass = Math.randomInRange(.5, 1), a.velocity = CAV.Vector3.create(), a.acceleration = CAV.Vector3.create(), a.force = CAV.Vector3.create()
		})();
		window.addEventListener("resize", function(a) {
			b(g.offsetWidth, g.offsetHeight);
			f.render(l)
		});
		b(g.offsetWidth, g.offsetHeight);
		c()
	}) : $("#wenkmPlayer").css("display", "none")
});
