...
myLib.desktop.lrBar={
	upLrBar:function(){
		var myData=myLib.desktop.getMydata()
			...
			,wh=myData.winWh;
		    $lrBar.css({'bottom':Math.floor((wh['h']-$lrBar.height())/2)-60});
...
;
		 			...
		 			$(".item").find('li')
		 				.click(function(){
		 					id=this.id;
		 					var ls_url=$(this).find('a').attr("href");
		 					var ls_title=$(this).find('a').text();

		 					myLib.desktop.win.newWin({
													 WindowTitle:ls_title,
													 iframSrc:ls_url,
													 WindowsId:id,
													 WindowAnimation:'none', 
													 WindowWidth:"730",
	...
	init:function(iconData){
		 ...
		 var data=[
					...
					,[{
					  text:"theme by小草",
					  func:function(){
					  	window.open('http://blog.yesfree.pw');
					  } 
					  }]
					,[{
					  text:"emlog强力驱动",
