...
<div id="m">
	...
	<div class="c">
		<?php foreach($commentStacks as $cid):
			...
			$comment['poster'] = $comment['url'] ? '<a href="/go.php?url='.urlencode($comment['url']).'" target="_blank">'.$comment['poster'].'</a>' : $comment['poster'];
		?>
		<div class="l">
	...
	<div class="c">
		...
		<p>欢迎回来，<?php echo $_COOKIE['name']; ?></p>
		昵称<br /><input type="text" name="comname" maxlength="30" value="<?php echo $_COOKIE['name'];?>" /><br />
		邮件地址 (选填)<br /><input type="text" name="commail" value="<?php echo $_COOKIE['mail'];?>" /><br />
		个人主页 (选填)<br /><input type="text" name="comurl" value="<?php echo $_COOKIE['url'];?>" /><br />
		<?php else: ?>
		昵称<br /><input type="text" name="comname" maxlength="30" value="<?php echo $_COOKIE['name'];?>" /><br />
		邮件地址 (选填)<br /><input type="text" name="commail" value="<?php echo $_COOKIE['mail'];?>" /><br />
		个人主页 (选填)<br /><input type="text" name="comurl" value="<?php echo $_COOKIE['url'];?>" /><br />
